...
public enum AppointmentStatus { Scheduled = 0, Cancelled = 1, Completed = 2 }
...
public class Appointment : Entity
{
    ...
    internal static Appointment CreateForCustomer(Guid scheduleId, DateTime startsAtUtc, TimeSpan duration, Customer customer, Guid serviceId, string serviceName, string? notes)
    {
        ...
        if (serviceId == Guid.Empty)
            throw new SchedulingException("Serviço obrigatório.");
        ...
        return new Appointment
        {
            ...
            CustomerPhone = phone,
            ServiceId = serviceId,
            ServiceNameSnapshot = serviceName?.Trim() ?? string.Empty,
    ...
    internal bool Reschedule(DateTime newStartUtc, TimeSpan newDuration)
    {
        if (Status != AppointmentStatus.Scheduled)
            throw new SchedulingException("Somente agendamentos ativos podem ser reagendados.");
    ...
    public bool UpdateNotes(string? notes)
    {
        ...
        if (string.Equals(Notes, normalized, StringComparison.Ordinal)) return false;
        Notes = normalized;
        Touch();
        return true;
    }
    ...
    public bool Cancel(string reason)
    {
        ...
        {
            if (string.Equals(CancellationReason, normalized, StringComparison.Ordinal)) return false;
    ...
    public bool Complete()
    {
        ...
        if (Status == AppointmentStatus.Cancelled)
            throw new SchedulingException("Não é possível concluir um agendamento cancelado.");
